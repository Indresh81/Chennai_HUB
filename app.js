const App = (() => {
  ...
  function renderProductArt(element, product) {
    element.className = `product-art ${product?.accent || ''}`;
    element.replaceChildren();

    if (product?.image) {
      element.removeAttribute('aria-hidden');
      element.append(createProductImage(product));
      return;
    }

    element.setAttribute('aria-hidden', 'true');
    element.textContent = product?.emoji || '📱';
  }
  ...
  function setProductSummary(product) {
    ...
    if (!product) {
      ...
      if (art) renderProductArt(art, null);
    ...
    if (art) renderProductArt(art, product);
